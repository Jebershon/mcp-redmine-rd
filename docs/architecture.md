...
All modules live under the `mcp_redmine_rd` package (`src/mcp_redmine_rd/`).
...
The package exposes a console entry point `mcp-redmine-rd` (defined in `pyproject.toml`) that calls `server:main`.
...
| `list_issues` | `view_issues` |
| `get_issue_relations` | `view_issues` |
...
| `enumerations/priorities` | `view_issues` |
